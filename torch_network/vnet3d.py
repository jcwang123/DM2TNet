...
class DownTransition(nn.Module):
    def __init__(self,
        ...
        self.attention_module = attention_module
        self.down_conv = nn.Conv3d(in_channels,
                                   out_channels,
                                   kernel_size=2,
                                   stride=2)
...
class ChannelGate(nn.Module):
    def __init__(self, in_channels, reduction_ratio=4):
        ...
        self.channel_gate_layers.append(
    ...
    def forward(self, x):
        avg_pool_out = F.avg_pool3d(x,
                                    kernel_size=(x.size(2), x.size(3),
                                                 x.size(4)))
        x_out = self.channel_gate(avg_pool_out)
        c_att = torch.sigmoid(x_out).unsqueeze(2).unsqueeze(3).unsqueeze(
            4).expand_as(x)
        out_att = x * c_att
        return x + out_att
    ...
    def __init__(self,
        ...
        self.spatial_gate_layers.append(
            nn.Conv3d(in_channels,
                      ...
                      kernel_size=1))
        self.spatial_gate_layers.append(
            nn.BatchNorm3d(in_channels // reduction_ratio))
        self.spatial_gate_layers.append(nn.ReLU())
        for i in range(dilated_conv_layers):
            self.spatial_gate_layers.append(
                nn.Conv3d(in_channels // reduction_ratio,
                          in_channels // reduction_ratio,
                          kernel_size=3,
                          padding=dilation,
                          dilation=dilation))
            ...
            self.spatial_gate_layers.append(nn.ReLU())
        self.spatial_gate_layers.append(
            nn.Conv3d(in_channels // reduction_ratio, 1, kernel_size=1))
...
class VNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.input_transition = InputTransition(8, prelu, dilation,
                                                attention_module)
        self.down_transition16 = DownTransition(8, num_layers, prelu, dilation,
                                                attention_module)
        self.down_transition32 = DownTransition(16, num_layers, prelu,
                                                dilation, attention_module)
        self.down_transition64 = DownTransition(32, num_layers + 1, prelu,
                                                dilation, attention_module)
        self.down_transition128 = DownTransition(64, num_layers + 1, prelu,
                                                 dilation, attention_module)
